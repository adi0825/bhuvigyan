...
$backendRoot = "C:\Users\athar\Desktop\Agri\backend"
$maven = "C:\Users\athar\Desktop\Agri\apache-maven-3.9.15\bin\mvn.cmd"

foreach ($s in $services) {
    Start-Process -FilePath "cmd.exe" -ArgumentList "/c cd /d $backendRoot; set SERVER_PORT=$($s.port); set SPRING_PROFILES_ACTIVE=local; $maven -pl $($s.dir) spring-boot:run" -PassThru | Out-Null
